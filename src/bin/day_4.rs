...
fn main() {
  ...
  let lines_vec = &binding
    .lines()
    .enumerate()
    .map(|(i, line)| (i, line.chars().enumerate().collect::<Vec<_>>()))
    .collect::<Vec<_>>();
  let mut counter = 0;
  for (line_index, line) in lines_vec.iter() {
    for (char_index, char) in line.iter() {
      if *char == 'X' {
...
fn get_surrounding_letter(
  line_index: &usize,
  char_index: &usize,
  lines: &Vec<(usize, Vec<(usize, char)>)>,
  wanted_letter: char,
  ...
  let surrounding_letters = surrounding_indexes
    .filter_map(|(x, y)| {
      ...
      let letter = lines.get(y_index)?.1.get(x_index)?;
      if letter.1 == wanted_letter {
        Some((x_index, y_index, letter.1))
      } else {
        None
      }
    })
